...
export default function DataMockup({ isActive = true }: { isActive?: boolean }) {
  const [mounted, setMounted] = useState(false);
  const [downMB, setDownMB] = useState(804.66);
  const [upMB, setUpMB] = useState(470.49);
  const [uptimeMinutes, setUptimeMinutes] = useState(42);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setDownMB(prev => prev + (Math.random() * 2.5 + 0.5));
      setUpMB(prev => prev + (Math.random() * 1.2 + 0.2));
    }, 2000);
  ...
  // Dynamic bar height for today based on ticking numbers
  const todayDarkHeight = Math.min(100, 5 + ((upMB - 470.49) / 20)).toFixed(1);
  const todayLightHeight = Math.min(100, 15 + ((downMB - 804.66) / 40)).toFixed(1);
          ...
          <div className="border border-[#121212] rounded-[24px] p-[16px] flex flex-col gap-[12px] bg-transparent">
            ...
            <div className="h-[180px] w-full mt-2 relative border border-[#121212] rounded-[16px] p-[12px] flex">
               ...
               <div className="flex-1 h-full relative flex items-end justify-around pb-[20px] px-[10px]">
                  ...
                  {/* Bars & X Labels */}
                  {[
                    { label: 'Jun 3', light: `${todayLightHeight}%`, dark: `${todayDarkHeight}%`, up: `${upMB.toFixed(2)} MB`, down: `${downMB.toFixed(2)} MB`, total: `${totalGB} GB` },
                    ...
                    return (
                      <div key={i} className="flex flex-col items-center h-full justify-end relative z-10 hover:z-50 group w-[32px]">
                        ...
                        <div className="w-full flex flex-col justify-end h-full cursor-pointer hover:brightness-110 transition-all">
                          <div className="w-full bg-[#1e3a8a] transition-all duration-1000 ease-[cubic-bezier(0.34,1.56,0.64,1)]" style={{ height: mounted ? bar.dark : '0%' }}></div>
                          <div className="w-full bg-[#60a5fa] transition-all duration-1000 ease-[cubic-bezier(0.34,1.56,0.64,1)]" style={{ height: mounted ? bar.light : '0%' }}></div>
                        </div>
